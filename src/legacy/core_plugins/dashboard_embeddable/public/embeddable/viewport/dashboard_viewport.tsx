...
import { EmbeddableFactory, ViewMode } from 'plugins/embeddable_api';
// @ts-ignore
...
export class DashboardViewport extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    const {
      ...
      title,
      description,
    } = this.props.container.getOutput();
  ...
  public componentDidMount() {
    this.mounted = true;
    this.unsubscribe = this.props.container.subscribeToOutputChanges(newOutput => {
      const {
        isFullScreenMode,
        panels,
        viewMode,
  ...
  public render() {
    const { getEmbeddableFactory, container } = this.props;
    return (
      <div
